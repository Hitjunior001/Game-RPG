...
def save_game(Hero):
    with open("saves/player_save.txt", "w") as file:
        ...
        file.write(f"\n")
        file.write(f"\n")
        file.write(f"{Hero.alive}\n")
        file.write(f"{Hero.special_attack}\n")
        file.write(f"{Hero.combat}\n")
...
def load_game():
    if os.path.exists("saves/player_save.txt"):
        with open("saves/player_save.txt", "r") as file:
            ...
            items =  list(file.readline().strip())
            equip_chest = list(file.readline().strip())
            equip_weapon =  list(file.readline().strip())
            alive = bool(file.readline().strip())
            special_attack = bool(file.readline().strip())
            combat = bool(file.readline().strip())
        return name, hp, maxHp, energy, maxEnergy, damage, kills, weapon, armor, golds, exp, level, up_level, x, y, style, items,  equip_chest, equip_weapon, alive, special_attack, combat
    else:
        return None
        ...
        for row_index, row in enumerate(map):
            for column_index, element in enumerate(row):
                if player_position[0] == row_index and player_position[1] == column_index:
                    ...
                    print("_ ", end="")
            print('MAPA NÃO ESTA EM FUNCIONAMENTO!!! ATENÇÃO')
...
class Mob(object):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 10, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False):
        self.hp = hp
        self.x = x 
        self.y = y
...
class Monk(Mob, Bag, Item):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 10, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False):
        super().__init__(self, name, hp = 110, maxHp = 110, energy = 300, maxEnergy = 300, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, style = 'meele', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
class Archer(Mob, Bag, Item):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 10, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, arrows = 0):
        self.arrows = arrows
        super().__init__(hp = 100, maxHp = 100, energy = 100, maxEnergy = 100, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, style = 'archery', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
def randomMob():
    ...
    if Map.place == 'Ground':
        if probability >= 0 and probability <=1:
            ...
            mob = Enemy('Zombie', 10,10,5, random_gold, random_exp)
            mob.mob_level()
            return mob
        elif probability >= 4 and probability <=5:
            mob = Enemy('Kid Zombie', 10,10, 5, random_gold, random_exp)
            mob.mob_level()
            return mob
        elif probability >= 6 and probability <=7:
            mob = Enemy('Wolfmen',15,15,8, random_gold, random_exp)
            mob.mob_level()
            return mob  
        elif probability >= 8 and probability <=10:
            mob = Enemy('Evil elfo',15,15,8, random_gold, random_exp)
            mob.mob_level()
            return mob
        elif probability >= 12 and probability <=15:
            mob = Enemy('Globin Sneaky', 5,5,3, random_gold, random_exp)
            mob.mob_level()
            return mob
        else:
            mob = Enemy('Globin Lancer', 5,5,3, random_gold, random_exp)
            mob.mob_level()
            return mob
...
def Main_menu():
    ...
    print('[3] '+ strings['main_menu_option3'])
    print(strings["divider"])
    option = input(strings['choice']+' ')
    if option == '1':
        return False
...
if __name__ == "__main__":
    ...
    Toothpick = Item('Toothpick', 'weapon','all', 2, 0, 0,0,2)
