...
def save_game(Hero):
    #ASSIM QUE O PLAYER SALVAR O JOGO, O MAPA VAI JUNTO.
    save_game_map(Map)
    with open("saves/player_save.txt", "w") as file:
        ...
        file.write(f"{Hero.damage_fix}\n")
        file.write(f"{Hero.armor_fix}\n")
...
def load_game():
    if os.path.exists("saves/player_save.txt"):
        with open("saves/player_save.txt", "r") as file:   
            ...
            damage_fix = int(file.readline().strip())
            armor_fix = int(file.readline().strip())
        ...
        if style == 'archery':
            return name, hp, maxHp, energy, maxEnergy, damage, kills, weapon, armor, golds, exp, level, up_level, x, y, space,damage_fix,armor_fix, style, arrows, items,  equip_chest, equip_weapon
        else:
            return name, hp, maxHp, energy, maxEnergy, damage, kills, weapon, armor, golds, exp, level, up_level, x, y, space,damage_fix,armor_fix, style, items,  equip_chest, equip_weapon

    else:
        return None
def save_game_map(Map):
    with open("saves/map_save.txt", "w") as file:
        file.write(f"{Map.place}\n")
        file.write(f"{Map.minute}\n")
        file.write(f"{Map.hours}\n")
        file.write(f"{Map.day}\n")
        file.write(f"{Map.mounth}\n")
        file.write(f"{Map.years}\n")
    with open("saves/map_places_save.bin", "wb") as f:
        # escreve a lista de itens no arquivo binário
        pickle.dump(Map.available_places, f)
def load_game_map():
    if os.path.exists("saves/map_save.txt"):
        with open("saves/map_save.txt", "r") as file:   
            place = file.readline().strip()
            minute = float(file.readline().strip())
            hours = int(file.readline().strip())
            day = int(file.readline().strip())
            mounth = int(file.readline().strip())
            years = int(file.readline().strip())
        with open("saves/map_places_save.bin", "rb") as f:
            available_places = pickle.load(f)
        return Map(place, minute, hours, day, mounth, years, available_places)
    else:
        return Map('Ground')
...
class Item():
    ...
    def __str__(self):
        if self.equipped == True:
            ...
            bag_description = strings['bag_description']
            return f'{self.name}\n{bag_description}\nGold: {self.gold}'
            ...
            if len(Hero.equip_weapon) == 1:
                ...
                if self.typer == 'chest':
                    ...
                    if self.style == Hero.style or self.style == 'all':
                        return f'{self.name}\nDamage: {bc.GREEN}{bc.BOLD}+{self.damage}{bc.ENDC}{bc.BOLD} 💪\nArmor: {self.armor} 🛡️\nFor: {bc.GREEN}{bc.BOLD}{self.style}{bc.ENDC}{bc.BOLD}\nGold: {self.gold}🟡\n{bc.GREEN}{can_equip}{bc.ENDC}{bc.BOLD}'
                    else:
                ...
                if self.typer == 'weapon':
                    ...
                    if self.style == Hero.style or self.style == 'all':
                        if armor > self.armor:
                            return f'{self.name}\nDamage: {self.damage} 💪\nArmor: +{bc.GREEN}{bc.BOLD}{armor - self.armor}{bc.ENDC}{bc.BOLD} 🛡️\nFor: {self.style}\nGold: {self.gold}🟡\nGold: {self.gold}🟡'
                        else:
                            return f'{self.name}\nDamage: {self.damage}💪\nArmor: -{bc.RED}{bc.BOLD}{self.armor - armor}{bc.ENDC}{bc.BOLD} 🛡️\nFor: {self.style}\nGold: {self.gold}🟡\nGold: {self.gold}🟡'
            else:
                if self.style == Hero.style or self.style == 'all':
                    return f'{self.name}\nDamage: +{self.damage} 💪\nArmor: +{self.armor} 🛡️\nFor: {bc.GREEN}{bc.BOLD}{self.style}{bc.ENDC}{bc.BOLD}\nGold: {self.gold}🟡\n{bc.GREEN}{can_equip}{bc.ENDC}{bc.BOLD}'
                else:
...
class Player(object):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 0,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        self.hp = hp
        self.space = space
        self.x = x 
        self.y = y
        ...
        self.weapon = weapon
        self.energy = energy
        self.damage_fix = damage_fix
        self.damage = damage
        self.armor_fix = armor_fix
        self.armor = armor
        self.kills = kills
        self.items = items
        self.alive = alive
    ...
    def Move(self):
        ...
        if probability >= 0 and probability <=13:
            ...
            if Map.place == 'Ground' and 'Dungeon' not in Map.available_places:
                Map.Enter('Dungeon')
            else:
               print(strings['move_msg'].format(Map.place))
        elif probability >= 24 and probability <=26:
            if Map.place == 'Ground' and 'Donjon' not in Map.available_places:
                Map.Enter('Donjon')
            else:
                print(strings['move_msg'].format(Map.place))
        elif probability >= 27 and probability <=28:
            if Map.place == 'Ground' and 'Tower_2' not in Map.available_places:
                Map.Enter('Tower_2')
            else:
                print(strings['move_msg'].format(Map.place))
        else:
    ...
    def level_up(self):
        ...
        self.up_level = round(self.up_level)
        self.exp = int(self.exp)
        self.up_level = int(self.up_level)
    ...
    def Attack1_E(self,target):
        ...
        if (attack <=10 and attack > 0):
            ...
            damage = f'{bc.GREEN}{self.damage}{bc.ENDC}{bc.BOLD}'
            print(strings["divider"])
            print(strings['basic_attack'].format(self.name,damage,target.name))
            print(strings["divider"])
            target.take_damage(self.damage)
        else:
            damage = f'{bc.GREEN}{self.damage*2}{bc.ENDC}{bc.BOLD}'
            print(strings["divider"])
    ...
    def take_damage(self, damage):
        if self.armor > 0:
            ...
            # 50 armor = 7.5%
            # 70 armor = 10.5%
            red_damage_percent = self.armor * 0.015
            red_damage = damage * red_damage_percent
            damage -= red_damage
    ...
    def stats(self):
        ...
        while True:
            ...
            print(strings['you_exp'].format(Hero.exp, Hero.up_level))
    ...
    def __str__(self):
        ...
        color_life = '\033[32m'  # verde para vida acima de 50%
        if health_percent <= 0.5:
            color_life = '\033[33m'  # amarelo para vida entre 0% e 50%
        if health_percent <= 0.25:
            color_life = '\033[31m'  # vermelho para vida abaixo de 25%
        energy_percent = self.energy / self.maxEnergy
        color_energy = '\033[32m'  # verde para vida acima de 50%
        if energy_percent <= 0.5:
            color_energy = '\033[33m'  # amarelo para vida entre 0% e 50%
        if energy_percent <= 0.25:
            color_energy = '\033[31m'  # vermelho para vida abaixo de 25%
        
        str_format = {
            'Warrior': lambda: f'{self.name} Warrior - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Damage: {self.damage}💪 Armor: {self.armor}🛡️',
            'Lancer': lambda: f'{self.name} Lancer - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Damage: {self.damage}💪 Armor: {self.armor}🛡️',
            'Archer': lambda: f'{self.name} Archer - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Arrows: {self.arrows}Damage: {self.damage}💪 Armor: {self.armor}🛡️',
            'Cid_Kagenou': lambda: f'{self.name} Shadow - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Damage: {self.damage}💪 Armor: {self.armor}🛡️',
            'Monk': lambda: f'{self.name} Monk - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Damage: {self.damage}💪 Armor: {self.armor}🛡️',
            'Dragontooth': lambda: f'{self.name} Dragontooth - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Energy: {color_energy}{self.energy}/{self.maxEnergy}{bc.ENDC}{bc.BOLD}⚡ Damage: {self.damage}💪 Armor: {self.armor}🛡️',
        }
        return str_format[type(self).__name__]()
#====================A PARTIR DAQUI SERÁ FEITO AS CLASS DE HEROIS HERDADO DE MOB=====================
#CLASS DO WARRIOR(GUERREIRO)
class Warrior(Player, Bag, Item):
    def __init__(self, name, hp = 150, maxHp = 150, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name, hp = 150, maxHp = 150, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'sword', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
...
class Lancer(Player, Bag, Item):
    def __init__(self, name, hp = 130, maxHp = 130, energy = 150, maxEnergy = 150, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name, hp = 130, maxHp = 130, energy = 150, maxEnergy = 150, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'lancer', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
    ...
    def Disable_Special_Attack(self):
        if self.special_attack == True:
            ...
            self.damage -= 50
#CLASS DO MONG(MONGE)
class Monk(Player, Bag, Item):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name, hp = 110, maxHp = 110, energy = 300, maxEnergy = 300, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'monk', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
class Dragontooth(Player, Bag, Item):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name, hp = 160, maxHp = 160, energy = 200, maxEnergy = 200, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'sharptooth', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
class Archer(Player, Bag, Item):
    def __init__(self, name, hp = 100, maxHp = 100, energy = 130, maxEnergy = 130, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', arrows = 0, items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name,hp = 100, maxHp = 100, energy = 100, maxEnergy = 100, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'archery', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
class Cid_Kagenou(Player, Bag, Item):
    def __init__(self, name, hp = 50, maxHp = 50, energy = 30, maxEnergy = 30, damage = 5, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = '', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False):
        super().__init__(name, hp = 50, maxHp = 50, energy = 15, maxEnergy = 15, damage = 0, kills = 0, weapon = 0, armor = 0, golds = 0,exp=0, level=0,up_level=30, x = 0, y = 0, space = 9,damage_fix = 0, armor_fix = 0, style = 'shadow', items = [], equip_chest = [], equip_weapon = [], alive = True,special_attack = False, combat = False, boss_killed = False)
        self.__dict__.update({
            "hp": hp,
            "maxHp": maxHp,
            "energy": energy,
...
class Enemy():
    ...
    def mob_level(self):
        if Hero.level == Hero.level:
            ...
            self.damage += self.level*2.5
            self.hp += self.level*2.5
            self.maxHp += self.level*2.5
    ...
    def Attack1_E(self,target):
        ...
        if (attack <=10 and attack > 0):
            ...
            damage = f'{bc.RED}{self.damage}{bc.ENDC}{bc.BOLD}'
            print(strings["divider"])
            print(strings['basic_attack'].format(self.name,damage,target.name))
            print(strings["divider"])
            target.take_damage(self.damage)
    ...
    def __str__(self):
        ...
        if health_percent <= 0.25:
            color_life = '\033[31m'  # vermelho para vida abaixo de 25%
        return f'{self.name} - Life: {color_life}{self.hp}/{self.maxHp}{bc.ENDC}{bc.BOLD}💗 Damage: {self.damage}💪 Level: {self.level}'
...
class Boss():
    ...
    def mob_level(self):
        if Hero.level == Hero.level:
            self.level = Hero.level 
            self.damage += self.level*1.2
            self.hp += self.level*1.2
            self.maxHp += self.level*1.2
    #FUNÇÃO NECESSESÁRIA DE REVIVER OS INIMIGOS SEMPRE QUE SAIR DE COMBATE.
    def revive(self):
        self.hp = self.maxHp
        return self.hp
...
def randomMob(map_place):
    ...
    gold_range = drop_data[map_place]["gold"]
    exp_range = drop_data[map_place]["exp"]

    random_gold = randint(*gold_range)
    random_exp = randint(*exp_range)
...
def BlackMarket(self):
    with open("resources/items/items_blackmarket.json") as f:
        items = json.load(f)
    category_items = items['all']
    while True:
        print(strings['divider'])
        print(strings['welcome_blackmarket'])
        print(strings['divider'])
        print(strings['you_golds'].format(self.golds))
        print(strings['want_buy'])
        printed_types = set()
